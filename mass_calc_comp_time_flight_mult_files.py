...
# Calculando a massa de um íon secundário a partir do seu tempo de voo para multiplos arquivos peaks.py de uma vez só
...
all_files = sorted(glob.glob("/home/ubuntu/Documents/Dissertation/Emissão_de_matéria_em_Cristais_Iônicos _Calculo_Tempo_de_Voo_de_Ions_Secundarios/1_LiF_6_MeV_Cu/python_programs/picos*.txt"))

for i in range(0, len(all_files)):
    data = np.loadtxt("/home/ubuntu/Documents/Dissertation/Emissão_de_matéria_em_Cristais_Iônicos _Calculo_Tempo_de_Voo_de_Ions_Secundarios/1_LiF_6_MeV_Cu/python_programs/picos" + str(i + 1)+ ".txt") #lendo arquivos de dados
    ...
    #print(df)
    #df.to_csv('Massa calculada a partir do tempo de voo' + str(i + 1) + '.txt', sep = '\t')
    
    with open('Massas Calculadas' + str(i + 1) + '.txt', 'w') as f:
        for k in ldelta_M:
    ...
    with open('Massas Calculadas_no_text' + str(i + 1) + '.txt', 'w') as f:
        for k in ldelta_M:
                for i in range(0, len(df['Massa ToF'])):
                        massa_tof = df['Massa ToF'][i]
                        for j in range(0, len(dfc['Massa Tabela'])):
